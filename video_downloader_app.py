...
def _format_duration(seconds: Optional[float]) -> str:
    """Return a human-friendly HH:MM:SS.s string for the provided seconds."""
    if seconds is None:
        return ""
    ...
    if abs(secs - round(secs)) < 1e-6:
        ...
        if secs_str.startswith("00") and len(secs_str) > 2 and secs_str[2] == ".":
            secs_str = secs_str[1:]
...
def _build_metadata_csv(entries: List[dict]) -> str:
    """Create a CSV string describing downloaded clips."""
    if not entries:
        return ""
    ...
    for entry in entries:
        clip_seconds = entry.get("clip_duration_seconds")
        if clip_seconds is None:
            clip_seconds_str = ""
        else:
            clip_seconds_str = f"{clip_seconds:.3f}".rstrip("0").rstrip(".")
        writer.writerow(
            [
        ...
        if str(row.get(status_column, "")).strip().lower() == "downloaded":
            ...
            if path_str:
                ...
                if saved_path.exists():
                    ...
                    clip_start_raw = str(row.get("Clip Start Time") or "").strip()
                    clip_end_raw = str(row.get("Clip End Time") or "").strip()
                    clip_start_seconds_meta = (
                        parse_time_to_seconds(clip_start_raw) if clip_start_raw else None
                    )
                    ...
                    metadata_entries.append(
                        {
                            ...
                            "clip_duration_display": _format_duration(clip_duration_seconds),
                        }
                    )
